    ...
    #        We want to be always listenning, hence the while True
            while True:
                ...
                if data == 'Closing':
                    ...
                    if TrigCamDelay>=3:
                        time.sleep(durationSequence-TrigCamDelay)
                    self.abortThread = False
                    self.shot_name = shot_name
                    print 'Go!'
                    
                    self.threadTakePicture = threading.Thread(target =self.takeCiceroPicture)
                    self.threadTakePicture.start()
