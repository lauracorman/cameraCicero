...
#        try:
        print "here"
        for i in range(2):
            nameA = 'Chameleon camera ' + str(i+1)
            cam = ChameleonCam(i,name=nameA)
            self.cameras.append(cam)
            #self.camerasHandles.append(i+len(LucamEnumCameras()))
            self.camerasHandles.append(i)  
        print "there"  
#                
#        except:
#            print 'No camera of type Chameleon'
#            self.ui.InformationCamera.append('No camera of type Chameleon' )
...
#                    self.currentCamera.timeout = int(2*(sequence_time + 10) * 1000)
                    ...
                    #J'essaie de mettre ma prise d'image CHameleon ici
                    print 'Go Chameleon Cam!'
                    cam = self.cameras[2] #ou 2 ?
                    im = cam.takeSinglePicture()
                    #La finir en recopiant ce quil faut de saveCiceroPicture 
